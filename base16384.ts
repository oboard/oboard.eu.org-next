...
function align(
  ...
  while (i < input.length) {
    ...
    while (offset >= tWidth) {
      offset -= tWidth;
      console.log(rest + (char >> offset) + tOffset);
...
export function toSource(input: Uint8Array | Uint16Array): string {
...
export function decode(input: string | Uint16Array) {
  ...
  console.log(input[length] - 0x3d00);
...
const encodedStr = "螥袞惢壥睯帀㴂"
const encoded = toUint16Array(encodedStr);
const decoded = decode(encoded);
const decodedStr = new TextDecoder().decode(decoded);
console.log(encodedStr);
console.log(encoded);
