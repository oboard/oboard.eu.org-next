...
const getCategoryColor = (category: string) => {
  switch (category) {
    case 'Web': return 'rgba(59, 130, 246, 0.2)'; // 蓝色
    case 'App': return 'rgba(16, 185, 129, 0.2)'; // 绿色
    case 'AI': return 'rgba(236, 72, 153, 0.2)';  // 粉色
    case 'Design': return 'rgba(245, 158, 11, 0.2)'; // 黄色
...
export default function InterestingAvatar() {
  ...
  // 预先计算轨道数据，避免在渲染期间计算
  const orbits = useMemo(() => ability.map((category, i) => {
    // 每个类别的基础旋转速度
  ...
  const allTechIcons = useMemo(() => ability.flatMap(category => {
    // 每个类别的基础旋转速度
