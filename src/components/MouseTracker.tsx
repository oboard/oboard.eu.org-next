...
export default function MouseTracker() {
  ...
  if (typeof window !== "undefined") {
    window.addEventListener("mousemove", (e) => {
      if (hoverList.length > 0) setMousePos({ x: e.clientX, y: e.clientY });
    });
  }
  ...
  return (
    <div
      className="fixed z-101 rounded-full pointer-events-none bg-blue bg-opacity-10 filter-blur transform -translate-x-1/2 -translate-y-1/2"
      style={{
        // top:
        ...
        top:
          hoverProps.y != 0
            ? (hoverProps.y + hoverProps.height / 2 + mousePos.y) / 2
            : mousePos.y,
        left:
          hoverProps.x != 0
            ? (hoverProps.x + hoverProps.width / 2 + mousePos.x) / 2
            : mousePos.x,
